...
- **`custom_data` 規格**（JSONB，Odoo 原生擴充位）：
  ...
  ```json
  {
    "kind": "headquarters" | "branch" | "role" | "independent",
    "parent_customer_id": "<uuid>",     // kind='branch' 指向公司；kind='role' 指向公司或分店
    "role": "contact" | "owner" | "buyer",  // 僅 kind='role' 有意義
    "default_driver_id": "<user-uuid>",  // 選填；覆蓋區域 tag 帶的預設司機
    "invoice_format": "<string>",         // 貨單形式（二聯/三聯/電子發票）；通常存 headquarters 層級
    "invite_token": "<uuid>"              // 僅 kind='branch'；一次生成、永久有效，用於產生 LINE LIFF 邀請連結
  }
  ```

  **`kind` 的四種**（本專案 B2B 蔬果批發的客戶結構）：

  | kind | 業務層級 | 真實 / 虛擬 | 範例 | Odoo `customer_type` / `is_company` |
  |---|---|---|---|---|
  ...
  **屬性的繼承規則**（跨層讀取時從自己往上追到 headquarters）：
  ...
  - 電子發票信箱 `email`：存在 headquarters
  - 路線／區域 tag：存在 branch（每家分店的送貨區域可能不同）
  - 地址 `contact_address`：存在 branch（實際送貨點）和 role（聯絡人住址，可選）
  - 聯絡電話 `phone`：branch 存市話、role 存個人手機
...
### 0.16 Customer ↔ CustomAppUser 的 onboarding 流程

#### 關係 cardinality

```
Customer  (N) ←──── customer_custom_app_user_rel ────→  (N)  CustomAppUser
  ▲                                                           ▲
  │ 任一 customer 可被 0..N 人代表                               │ 一人可代 0..N customer
  │ 0 人代表 = 離線客戶（電話訂購），customer 資料存在但 Ordering 上沒人能登入它
  │                                                           │
  │                                                           └─ 0 代表「受邀但未接受」或「剛建帳號但沒綁客戶」
  │                                                           （Ordering 登入後檢查，0 筆 rel 就導「等待授權」頁）
```

**rel 上沒有 role/primary 欄位**：純多對多。代下單時由 UI 讓使用者選「為誰下」並記錄於 `sale_orders.custom_data.placed_by_custom_app_user_id`（見 §0.10）。

#### Onboarding 走業務人工分享連結（LINE LIFF 邀請制）

客戶**不能自助註冊**。業務員在 Admin 建好客戶資料後，**從 Admin 複製每家分店的邀請連結**，手動（如 LINE 訊息、簡訊）傳給客戶的採購窗口。

**無邀請信，無 AI GO invitations API**。

#### 邀請連結格式

```
{ORDERING_LIFF_URL}?token={invite_token}
```

- `ORDERING_LIFF_URL`：LINE LIFF 的進入點 URL，存於 `ir_config_parameters.ordering.liff_url`，deploy 時由 Admin 設定
- `invite_token`：UUID4，由 `create_customer_bundle` 在分店記錄建立時一併生成，存於 `branch.custom_data.invite_token`
- **Token 永久有效、可多次使用**（同一分店可傳給多位採購窗口）
- 若需要廢除（人員異動），Admin 使用 `customer_representative_revoke` 撤銷 rel；token 不需要更換

#### Admin UI 操作

- 客戶列表中，每家分店列一個「複製邀請連結」按鈕
- 點擊後將 `{LIFF_URL}?token={invite_token}` 複製到剪貼簿
- LIFF URL 由前端透過 `system_config_get` action 取得（`ordering.liff_url` key）

#### 建客戶表單欄位 → 資料落點

一張表單填完，後端 Action `create_customer_bundle` **同時建立 4 筆 `customers` 記錄**（公司 / 分店 / 聯絡人 / 負責人）：

| 表單欄位 | 落在哪筆 customer | 欄位 |
|---|---|---|
| 公司營業登記名 | **公司** (kind=headquarters) | `name` |
| 統編 | 公司 | `vat` |
| 公司負責人姓名 | **負責人** (kind=role, role=owner, parent=公司) | `name` |
| 店名 | **分店** (kind=branch, parent=公司) | `name` |
| 地址 | 分店 | `contact_address` + `state_id`/`city`/`zip_code` |
| 店內市話 | 分店 | `phone` |
| 店內聯絡人姓名 | **聯絡人** (kind=role, role=contact, parent=分店) | `name` |
| 聯絡電話 | 聯絡人 | `phone` |
| 電子發票信箱 | 公司 | `email` |
| 結帳方式 | 公司 | `payment_term` |
| 業務員 | 公司 | `salesperson_id` |
| 貨單形式 | 公司 | `custom_data.invoice_format` |
| 路線（區域） | 分店（打 tag） | `customer_tag_rel`（region tag） |

#### `create_customer_bundle` Action 的步驟

```
1. 建 customer C (kind=headquarters)：營業登記名、統編、業務、結帳、貨單、電子發票信箱
2. 生成 invite_token = uuid4()
   建 customer S (kind=branch, parent=C.id, invite_token=invite_token)：店名、地址、市話
3. 建 customer I1 (kind=role, role=contact, parent=S.id)：聯絡人姓名、手機（可選）
4. 建 customer I2 (kind=role, role=owner, parent=C.id)：負責人姓名（可選）
5. 建 customer_tag_rel：S ↔ 區域 tag（可選）
6. 回傳 { headquarters_id, branch_id, invite_token }
```

#### 客戶點邀請連結後（LIFF 流程）

```
7. 客戶點連結 → 瀏覽器開啟 LINE OA 加好友頁 → 加入後跳轉 LIFF
8. LIFF SDK 初始化，取得 LINE user profile（LINE user ID）
9. Ordering App 以 LINE user ID 呼叫 AI GO custom-app-auth，建立 / 登入 custom_app_user P
   （若 P 已存在則直接取得其 JWT）
10. Ordering App 讀取 URL 中的 ?token 參數
    → 呼叫 Action redeem_invite_token(token)
    （ctx.user = P 的 custom_app_user_id）
11. Action 找到 token 對應的分店 S 及其上層公司 C
    建立 2 筆 rel：
      P ↔ S（可代分店下單）← 主要使用
      P ↔ C（可代公司，用於業績歸屬查詢）
12. 回傳 { customer_id: S.id, headquarters_id: C.id }
    Ordering App 直接進入 S 的訂購頁，流程完成
```

> **為什麼只建 2 筆 rel（不含 I1, I2）**：`role` 類型的 customer 是虛擬容器，沒有訂單流程；只有 `branch` 和 `headquarters` 需要 rel 來支援訂單歸屬和業績計算。如需後續精細管控（多採購窗口分工），可由 Admin 手動建立額外 rel。

> **多人使用同一 token 是設計內行為**：同一家分店的兩位採購員都能點同一條連結各自完成 LIFF 流程，兩人都會取得該分店的 rel，互不影響。

#### 離職 / 換人 / 停用

| 情境 | 操作 |
|---|---|
| 採購員 P 離職 | Admin UI → `customer_representative_revoke`：刪除 P↔C, P↔S；P 的 custom_app_user 保留 |
| 需要加入新採購員 | 業務直接再發同一條邀請連結給新人；新人走 LIFF 流程自動建 rel |
| 公司停用（不再合作） | C/S/I1/I2 `active=false`；相關 rel 保留（歷史訂單追溯） |
| 分店關閉但公司還在 | 只停用 S（`active=false`）及 S 的 rel；C/I2 保留 |
...
-- 實務用 db.query 包裝
SELECT cau.*
FROM customer_custom_app_user_rel rel
JOIN custom_app_users cau ON cau.id = rel.custom_app_user_id
WHERE rel.customer_id = :customer_id;
```

對應 Action: `list_customer_representatives(customer_id)` — 在 §2.11 Admin 清單新增。

#### 查詢某 custom_app_user 可代表的所有 customer

```sql
SELECT c.*, c.custom_data -> 'kind' AS kind
FROM customer_custom_app_user_rel rel
JOIN customers c ON c.id = rel.customer_id
WHERE rel.custom_app_user_id = :ctx_user_id
  AND c.active = true;
```

對應 Action: `my_customers_list` — 在 §2.11 Ordering 清單新增（取代原本的 `my_profile_get`，因為「我」可能代多家 customer 不只一個 profile）。
...
| Action | 讀寫 | 操作 |
|---|---|---|
| `redeem_invite_token` | W | 以 URL 中的 `invite_token` 找到分店 S 及其公司 C，為 `ctx.user`（已登入的 custom_app_user P）建立 P↔S 和 P↔C 兩筆 rel；token 永久有效，可重複呼叫（已存在 rel 則跳過）；回傳 { customer_id, headquarters_id } |
| `my_customers_list` | R | 列出 `ctx.user` 可代表的所有 customer（rel 關聯，含 kind 標註），前端 UI 讓使用者選「為誰下單」 |
| `my_profile_get` | R | 回傳當前選定的 customer 資料（需帶 `customer_id` 參數並驗證 rel） |
...
| Action | 讀寫 | 操作 |
|---|---|---|
| `list_customers_for_me` | R | 業務看自己 salesperson；主管看整團；管理階層看全部；預設 filter `kind IN ('headquarters','independent','branch')`（隱藏 role 虛擬容器；用「顯示角色容器」開關切換） |
| `create_customer_bundle` | W | **一次建公司(headquarters)+分店(branch)+聯絡人(role)+負責人(role) 4 筆 customer**，打區域 tag；生成 `invite_token` 存入分店 `custom_data`（見 §0.16） |
| `customer_crud` | W | 建/改/刪單一 customer + custom_data（kind / parent_customer_id / role / invoice_format / default_driver_id） |
| `list_customer_representatives` | R | 查某 customer 綁了哪些 custom_app_user |
| `customer_representative_revoke` | W | 移除 customer ↔ custom_app_user 綁定（離職用） |
