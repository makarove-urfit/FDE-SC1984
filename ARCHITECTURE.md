...
- **`custom_data` 規格**（JSONB，Odoo 原生擴充位）：
  ...
  ```json
  {
    "kind": "headquarters" | "branch" | "individual" | "independent",
    "parent_customer_id": "<uuid>",     // kind='branch' 指向公司；kind='individual' 指向公司或分店
    "individual_role": "contact" | "owner" | "buyer" | null,  // 僅 kind='individual' 有意義
  ...
  **`kind` 的四種**（本專案 B2B 蔬果批發的客戶結構）：
  ...
  | kind | 業務層級 | 範例 | is_company |
  |---|---|---|---|
  | `headquarters` | 公司（法人本體） | 「家樂福股份有限公司」 | true |
  | `branch` | 分店（營運據點） | 「家樂福 - 內湖店」 | false（掛在公司統編下） |
  | `individual` | 個人（聯絡人 / 負責人） | 店內聯絡人「王小明」 | false |
  | `independent` | 獨立客戶（既是下單單位也是法人，無階層） | 個人戶 | 可 true/false |

  **屬性的繼承規則**（跨層讀取時從自己往上追到 headquarters）：
  - 統編 `vat`：存在 headquarters；branch/individual 查詢時向上追
  ...
  - 地址 `contact_address`：存在 branch（實際送貨點）和 individual（聯絡人住址，可選）
  - 聯絡電話 `phone`：branch 存市話、individual 存個人手機
...
#### Onboarding 走業主邀請制（選 C）

客戶**不能自助註冊**。業務員在 Admin 建好完整客戶資料後，邀請客戶聯絡人加入 Ordering。
...
| 統編 | 公司 | `vat` |
| 公司負責人姓名 | **負責人** (kind=individual, role=owner, parent=公司) | `name` |
| 店名 | **分店** (kind=branch, parent=公司) | `name` |
| 地址 | 分店 | `contact_address` + `state_id`/`city`/`zip_code` |
| 店內市話 | 分店 | `phone` |
| 店內聯絡人姓名 | **聯絡人** (kind=individual, role=contact, parent=分店) | `name` |
| 聯絡電話 | 聯絡人 | `phone` |
| 電子發票信箱 | 公司 | `email` |
...
2. 建 customer S (kind=branch, parent=C.id)：店名、地址、市話
3. 建 customer I1 (kind=individual, role=contact, parent=S.id)：聯絡人姓名、手機
4. 建 customer I2 (kind=individual, role=owner, parent=C.id)：負責人姓名
5. 建 customer_tag_rel：S ↔ 區域 tag
6. 發 invitation email 給聯絡人 I1 的 email
```

#### 受邀者接受邀請後

```
7. 受邀者點 email 連結 → LINE Login / 帳密註冊 → AI GO 建 custom_app_user P
8. AI GO (或 Ordering backend bridge) 建立 4 筆 rel：
     P ↔ C（可代公司）
     P ↔ S（可代分店下單）← 實務上主要用這筆
     P ↔ I1（代他自己）
     P ↔ I2（代公司負責人）
9. P 登入 Ordering 後 UI 列出「可代表的客戶」，預設選 S 下單
```

> **為什麼 4 筆 rel 都綁同一個 P**：實務上客戶單位下單都由這一位採購員操作；若將來公司擴編多人採購，再分別邀請各自的 email，新建的 custom_app_users 再依需要綁 rel（例如新增「副採購」只綁 P2 ↔ S）。

#### 離職 / 換人 / 停用

| 情境 | 操作 |
|---|---|
| 採購員 P 離職 | Admin UI 刪除 P 的全部 rel（P↔C, P↔S, P↔I1, P↔I2）；P 的 custom_app_user 保留（他可能代其他公司） |
| 換新採購員 | 發 invitation 給新 email → 建 P2 → 建新 rel；舊 P 的 rel 可選擇保留或清除 |
| 公司停用（不再合作） | C/S/I1/I2 `active=false`；相關 rel 可選保留（方便歷史訂單追溯）或刪除 |
...
| Action | 讀寫 | 操作 |
|---|---|---|
| `list_customers_for_me` | R | 業務看自己 salesperson；主管看整團；管理階層看全部；預設 filter `kind != 'individual'`（用「含個人」開關切換） |
| `create_customer_bundle` | W | **一次建公司+分店+聯絡人+負責人 4 筆 customer**，打區域 tag；發邀請信（見 §0.16） |
| `customer_crud` | W | 建/改/刪單一 customer + custom_data（kind / parent_customer_id / individual_role / invoice_format / default_driver_id） |
