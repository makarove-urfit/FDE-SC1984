...
3. 建 customer I1 (kind=role, role=contact, parent=S.id)：聯絡人姓名、手機
4. 建 customer I2 (kind=role, role=owner, parent=C.id)：負責人姓名
...
| `create_customer_bundle` | W | **一次建公司(headquarters)+分店(branch)+聯絡人(role)+負責人(role) 4 筆 customer**，打區域 tag；發邀請信（見 §0.16） |
